...
export class Timetable {
    ...
    render() {
        ...
        if (isAdmin) {
            adminToolbar.innerHTML = `
                ...
                <button id="reset-timetable-btn" class="glass-button" style="background: rgba(239, 68, 68, 0.1); color: var(--danger); border-color: rgba(239, 68, 68, 0.2); font-size: 0.8rem;">🔄 Reset Schedule</button>
                <button id="bulk-import-btn" class="glass-button" style="background: var(--bg-primary); color: var(--text-primary); border-color: var(--glass-border); font-size: 0.8rem;">📤 Bulk Import</button>
        ...
        if (isAdmin) {
            ...
            adminToolbar.querySelector('#reset-timetable-btn').onclick = () => this.resetTimetable(gridContainer, isAdmin);
            adminToolbar.querySelector('#bulk-import-btn').onclick = () => { window.location.hash = ROUTES.TIMETABLE_BULK; };
    ...
    resetTimetable(container, isAdmin) {
        Modal.confirm(
            'Reset Timetable?', 
            'This will clear all entries and reset time slots to default (9:00 AM - 4:30 PM pattern). This cannot be undone.',
            async () => {
                this.timeSlots = [
                    '09:00 - 10:30',
                    '10:30 - 12:00',
                ...
                this.gridData = {};
                this.renderGrid(container, isAdmin);
                Toast.info('Schedule reset to defaults. Don\'t forget to save!');
            }
        );
    }
}
