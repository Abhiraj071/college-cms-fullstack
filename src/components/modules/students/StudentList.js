...
export class StudentList {
    ...
    render() {
        ...
        header.style.alignItems = 'center'; // Changed from flex-end to center for better balance
        ...
        filterBar.innerHTML = `
            ...
            <div style="margin-left: auto; align-self: flex-end; padding-bottom: 4px;">
                <button id="resetFilters" style="background:transparent; border:none; color:var(--text-secondary); cursor:pointer; font-size: 0.8rem; font-weight: 600; text-decoration: underline; padding: 8px 0;">Reset Filters</button>
            </div>
        `;
        ...
        const updateYears = (courseName) => {
            ...
            const course = this.courses.find(c => (c.name || '').trim().toLowerCase() === courseNameLower);
        ...
        const loadStudents = async () => {
            ...
            try {
                ...
                const filtered = all.filter(s => {
                    ...
                    const filterCourse = (course || '').trim().toLowerCase();
                    if (studentCourse !== filterCourse) return false;
                    if (semester) return Number(s.semester) === Number(semester);
                    if (year) return (Number(s.semester) === (Number(year) * 2) - 1 || Number(s.semester) === (Number(year) * 2));
                    return true;
                }).sort((a, b) => {
