...
exports.createStudent = async (req, res) => {
    try {
        const { username, name, rollNo, course, semester, phone, joinDate } = req.body;

        // Auto-generate a guaranteed-unique internal email (never shown to user)
        const email = `${rollNo}.${Date.now()}.${Math.random().toString(36).substring(7)}@student.internal`;

        if (!username || !name || !rollNo || !course || !semester) {
            return res.status(400).json({ message: 'username, name, rollNo, course and semester are required' });
        }

        // 1. Check if username already exists
        let userInstance = await User.findOne({ username });
        if (userInstance) {
...
exports.createBulkStudents = async (req, res) => {
    try {
        ...
        for (const studentData of students) {
            try {
                const { name, rollNo, phone } = studentData;
                const username = rollNo;
                // Auto-generate guaranteed-unique internal email
                const email = `${rollNo}.${Date.now()}.${Math.random().toString(36).substring(7)}@student.internal`;

                let userInstance = await User.findOne({ username });
                if (userInstance) {
                    // Check if this user is orphaned (has no student record)
                    const studentExists = await Student.findOne({ userId: userInstance._id });
                    if (studentExists) {
                        results.failed++;
                        results.errors.push(`${name} (${rollNo}): Username/Student already exists`);
                        continue;
                    }
                    // If orphaned, we'll continue and create the Student record for this existing user
                } else {
                    const rollExists = await Student.findOne({ rollNo });
                    if (rollExists) {
                        results.failed++;
