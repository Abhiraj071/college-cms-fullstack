...
import { ResultPage } from '../components/modules/exams/ResultPage.js';
...
export class Router {
    constructor() {
        this.routes = {
            ...
            // Exams
            [ROUTES.EXAMS]: { roles: ALL, component: ExamList },
    ...
    getRouteInfo(hash) {
        ...
        const examsResultsPrefix = ROUTES.EXAMS_RESULTS.split('/:')[0];
        if (path.startsWith(examsResultsPrefix + '/')) {
            return {
                path,
                params,
                config: {
